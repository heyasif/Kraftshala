...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayTotalDetails = (data) => {
        ...
        const dateTimeContainer = document.getElementById('dateTime'); // Get the date and time container
    
        const currentDateTime = new Date();
        const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric' };
        const formattedDateTime = currentDateTime.toLocaleDateString('en-US', options);
    
        dateTimeContainer.innerHTML = `<h2>${formattedDateTime}</h2>`; // Set the date and time
    ...
    const getWeatherByLocation = async () => {
        const success = async (position) => {
            try {
                ...
                localStorage.setItem('weatherAppLocation', JSON.stringify({ latitude, longitude }));
    ...
    const toggleMode = () => {
        ...
        if (body.classList.contains('light-mode')) {
            modeToggleButton.innerHTML = '<i class="fas fa-moon"></i> Dark Mode';
            localStorage.setItem('weatherAppMode', 'light');
        } else {
            modeToggleButton.innerHTML = '<i class="fas fa-sun"></i> Light Mode';
            localStorage.setItem('weatherAppMode', 'dark');
        }
    };
